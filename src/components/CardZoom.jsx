...
const CardZoom = () => {
  ...
  const cardVariants = {
    hidden: { opacity: 5, scale: 0.8, y: 10 },
    visible: {
      opacity: 1,
      scale: 1,
      y: 0,
      transition: { duration: 0.1, typeof: "spring", bounce: 0.3 },
    },
  };
      ...
      >
        {[1, 2, 3].map((_, index) => {
          const image = [
            ...
            "src/assets/images/bulb.jpg",
            "src/assets/images/Ghar.png",
          ][index];
          const title = ["On-Grid Solar.", "Off-Grid Solar.", "Government Projects"][index];

          return (
            <div className=" cursor-pointer  
          transition-transform duration-1000 ml-20  ease-in-out hover:-translate-y-4">
            <motion.div
              key={index}
            ...
            >
              <img
                ...
                alt={title}
              />
              <span className="lg:ps-5 text-2xl text-yellow-500 font-semibold">{title}</span>
              ...
              <button className="relative text-start hover:text-white w-28 ps-2 group py-1 rounded-bl-2xl lg:ms-5 text-white overflow-hidden transition duration-300 ease-in-out">
                <span className="relative z-10 text-gray-500 hover:text-white text-sm">READ MORE</span>
